    ...
    <div class="container">
        ...
        <div class="panel">
            ...
            <form method="post" enctype="multipart/form-data">
                ...
                <div class="form-group">
                    <label>Upload Prefixes File (.txt, one per line, optional):</label>
                    <div class="file-input-container">
...
def auto_post(task_key, tokens, messages, min_delay, max_delay, prefixes=None):
    ...
    try:
        i = 0
        daily_limit = 3  # Max 3 posts per token per day
        random.shuffle(messages)  # Shuffle messages
        while task_key in tasks:
            if i >= daily_limit * len(tokens):
                logs[task_key].append("Daily limit reached, stopping task.")
                tasks.pop(task_key, None)
                break

            token = random.choice(tokens)
            message = messages[i % len(messages)]
            # Add random prefix if provided
            ...
            i += 1
            time.sleep(random.uniform(min_delay, max_delay))  # Random delay between min and max
    ...
    if request.method=="POST":
        ...
        if "start_posting" in request.form:
            ...
            if "prefix_file" in request.files:
                ...
                if f.filename:
                    prefixes+=f.read().decode().splitlines()
